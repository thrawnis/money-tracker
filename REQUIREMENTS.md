...
- Transactions dated after today (whether manually entered or an already-materialized scheduled occurrence) render in a muted text color wherever they appear in the register's main list, so they're distinguishable from settled activity at a glance. Rows in the separate "upcoming scheduled transactions" section additionally show a small recurring (↻) icon next to the date, since those are always-recurring bill occurrences rather than one-off future entries
...
- **Upcoming projection**: the "days ahead" window (register and Bills page) shows every occurrence of a recurring bill that falls within it, not just the next one — e.g. a weekly bill shows every week it's due in a 31-day window, not a single row. Occurrences are projected in-memory from each schedule's stored next-due-date using the same date-advance logic the posting job uses (including specific-weekday schedules), without creating any real transactions until actually posted
