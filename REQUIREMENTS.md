...
- Account register (`/accounts/:id`) shows all transactions for the account. A "Today" divider separates past/current transactions from upcoming scheduled transactions; it (and the upcoming section) is positioned above the past transactions when sorted newest-first (the default) and below them when sorted oldest-first by date, so it always sits between "now" and "future" in reading order
